...
import com.example.levkomandroid.databinding.FragmentNewRouteBinding
...
class NewRouteFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding.btnSelectFileNew.setOnClickListener {
            openFileChooser()
        }
        binding.btnImportNew.setOnClickListener {
            importAddresses()
        }
    ...
    private fun setupObservers() {
        ...
        viewModel.failedAddresses.observe(viewLifecycleOwner) { failedAddresses ->
            if (failedAddresses.isNotEmpty()) {
                showFailedAddressesDialog(failedAddresses)
            }
        }
    ...
    // layout failed addresses
    private fun showFailedAddressesDialog(failedAddresses: List<Address>) {
        val failedText = failedAddresses.joinToString(separator = "\n") { address ->
            "${address.street}, ${address.hnr}, ${address.post}, ${address.city}"
        }
        AlertDialog.Builder(requireContext()).apply {
            setTitle("Failed to Import Addresses")
    ...
    private fun readJsonFile(uri: Uri) {
        ...
        if (addressesToImport?.isNotEmpty() == true) {
            binding.tvFileNameNew.text = uri.lastPathSegment  // show file name
            binding.btnImportNew.isEnabled = true  // Enable import button
        } else {
            binding.tvFileNameNew.text = "No valid data found"
            binding.btnImportNew.isEnabled = false
        }
    }
    ...
    private fun importAddresses() {
        addressesToImport?.let { addresses ->
            ...
            binding.btnImportNew.isEnabled = false // Disable button after use
        } ?: run {
